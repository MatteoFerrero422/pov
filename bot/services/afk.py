...
async def settle_activity(conn,user_id):
    ...
    raw_elapsed=max(0,(now-last).total_seconds())
    # AFK income/resources can never accumulate for more than one hour.
    elapsed=min(raw_elapsed, MAX_AFK_SECONDS)
