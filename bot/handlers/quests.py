...
from services.quests import format_stars, progress as quest_progress

router = Router()

CHANNEL_CHAT_ID = '@Pepexspace'
SUBSCRIPTION_QUEST_ID = 4
...
@router.callback_query(F.data == 'quest:check_sub')
async def cb_check_subscription(callback: CallbackQuery):
    async with db.connect() as conn:
        user = await queries.get_user_by_telegram_id(conn, callback.from_user.id)
        if not user: return await callback.answer('Сначала войдите', show_alert=True)

        rows = await queries.get_quest_rows(conn, user.id)
        quest = next((r for r in rows if r['id'] == SUBSCRIPTION_QUEST_ID), None)
        if quest and quest['claimed']:
            return await callback.answer('Это задание уже выполнено ✅', show_alert=True)

        try:
            member = await callback.bot.get_chat_member(CHANNEL_CHAT_ID, callback.from_user.id)
        except TelegramForbiddenError:
            return await callback.answer(
                '❌ Бот не может проверить подписку на канал. Добавьте бота администратором канала и попробуйте снова.',
                show_alert=True,
            )
        except TelegramBadRequest:
            return await callback.answer(
                '❌ Не удалось проверить канал. Проверьте, что бот добавлен в канал и username канала указан верно.',
                show_alert=True,
            )
        except Exception:
            return await callback.answer(
                '❌ Ошибка проверки подписки. Попробуйте ещё раз через несколько секунд.',
                show_alert=True,
            )

        status = getattr(member.status, 'value', member.status)
        if status not in ('member', 'administrator', 'creator'):
            return await callback.answer('❌ Вы ещё не подписаны на канал.', show_alert=True)

        got_reward = await quest_progress(conn, user.id, SUBSCRIPTION_QUEST_ID, 1)
        await conn.commit()
        await callback.answer('🎉 Подписка подтверждена! Награда начислена.' if got_reward else 'Подписка подтверждена ✅', show_alert=True)
        text = await quest_text(conn, user.id)
    await callback.message.edit_text(text, reply_markup=quests_keyboard(), parse_mode='HTML')
