...
async def cb_business_view(callback,state):
    ...
    if info.get('max_level') == 7:
        level_info='\n'.join(
            f"L{lvl}: {fmt(BUSINESS_LEVELS[lvl]['income_money'])}$/час"
            + (f" + {fmt(BUSINESS_LEVELS[lvl]['income_stars'])} ⭐/час" if BUSINESS_LEVELS[lvl]['income_stars'] else '')
            + ("  ← уровень покупки" if lvl == purchase_level else '')
            for lvl in range(1,8)
        )
    else:
        level_info='\n'.join(f"L{lvl}: {fmt(info.get('income_by_level',{}).get(lvl,0))}$/час" for lvl in range(1,info.get('max_level',1)+1))
